...
export default function About() {
  ...
  return (
    ...
    >
      <motion.div
        style={{ y: wordY }}
        className="absolute -left-10 top-1/2 -translate-y-1/2 text-[18rem] sm:text-[26rem] font-bold text-black/[0.025] select-none pointer-events-none whitespace-nowrap"
        aria-hidden
      >
        ABOUT
      </motion.div>
      ...
      <motion.div
        style={{ y: numberY }}
        className="absolute right-8 sm:right-16 top-32 font-mono text-xs tracking-[0.3em] text-black/30 pointer-events-none"
        aria-hidden
      >
        01 / 05
      </motion.div>
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-16 items-start">
          ...
          <div className="md:col-span-8">
            ...
            <motion.p
              ...
              custom={5}
              variants={reveal}
              className="text-lg text-black/70 leading-relaxed max-w-2xl mb-12"
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-8 max-w-3xl">
              {stack.map((group, i) => (
                ...
                >
                  <p className="font-mono text-[10px] text-black/50 tracking-[0.25em] uppercase mb-4">
                    {group.label}
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {group.items.map((item) => (
                      <span
                        key={item}
                        className="px-2.5 py-1 text-xs font-mono border border-black/10 rounded-full text-black/80 hover:border-black/30 hover:bg-black/[0.04] transition-colors"
                      >
                        {item}
                      </span>
                    ))}
        ...
        >
          <p className="font-mono text-[10px] text-black/50 tracking-[0.25em] uppercase mb-6">
            Activity
          </p>
